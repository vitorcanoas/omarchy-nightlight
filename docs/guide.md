...
Omarchy Night Light adds per-monitor colour temperature to the Omarchy bar.
Each screen can have its own blue-light filter intensity, software brightness,
pause timer and daily schedule through one widget.

The plugin talks to the DBus objects exposed by `wl-gammarelay-rs`. The colour
temperature scale follows Windows Night light:

```text
Kelvin = 6500 - (53 x percent)
0% = 6500K neutral
100% = 1200K deep amber
```
...
`yay -S wl-gammarelay-rs` is an equivalent alternative when `yay` is already
installed. Ubuntu appears in this repository only as the operating system used
...
The widget calls the CLI by absolute path, so `install.sh` is not needed for
normal plugin use. Run it only if you want a short `omarchy-nightlight` command
on `PATH` or need to call the CLI from an integration:

```bash
~/.config/omarchy/plugins/vitorcanoas.nightlight/install.sh
```

The installer checks `wl-gammarelay-rs` and creates a symlink in
`${XDG_BIN_HOME:-$HOME/.local/bin}`. It does not start the daemon, change a
display, install the plugin or modify Omarchy configuration.
...
Keyboard controls in the panel:

- `j` / `k`: move between rows
- `h` / `l`: change the current value
- `Space` / `Enter`: flip a switch
- `Esc`: close

### Software brightness

The drawer's brightness slider scales the compositor's gamma ramp; it does not
dim the monitor's physical backlight. Omarchy's
`omarchy-brightness-display` is preferable when a monitor supports DDC/CI. This
control works on every output and can dim only the selected screen. The minimum
is 10%.
...
that must never launch the daemon:
...
If `install.sh` was used, the commands can instead use the short
`omarchy-nightlight` name. Check the current bindings before choosing keys.

The daemon starts on demand, but saved temperatures need to be re-applied once
per session. Use the absolute plugin path:
...
`restore` only touches outputs with a saved line in the config file, so it is
...
screen's own intensity automatically.

**Schedule** turns the filter on and off at two clock times. It does not
calculate sunset and does not use the network. Both controls are QML timers
inside the plugin, so removing the plugin leaves no systemd timer or cron job
behind.
...
Backend state is stored in `~/.config/omarchy/nightlight.conf`:
...
screen state. This is why `off` does not erase saved values and `on` can restore
them. UI settings such as `command`, `nightPercent`, schedule values and pause
state remain inline on the plugin entry in `shell.json`.
...
This example requires the optional installer and `jq`. Without the installer,
replace both command occurrences with the absolute plugin path. Keep
...
added manually. Runtime lock directories live in `XDG_RUNTIME_DIR` and
...
Check that `wl-gammarelay-rs` is installed and that `hyprsunset` is not holding
the output's gamma control.

### A slider moves but the screen does not change

Stop the competing provider:

```bash
pkill hyprsunset
```

### A screen is stuck dark or warm

If the plugin is installed, run `reset`. Otherwise stop
`wl-gammarelay-rs` or log out.

### A screen is missing

The panel lists outputs exposed by:

```bash
busctl --user tree rs.wl-gammarelay
```

Reconnect the monitor or restart the daemon.

### The widget does not appear

Rescan and inspect the plugin:
...
make shellcheck  # shell static analysis
...
- `Panel.qml`: bar widget, panel UI and shell integration
- `Model.js`: state, percentages and schedule calculations
- `bin/omarchy-nightlight`: CLI, DBus operations and daemon
- `manifest.json`: Omarchy plugin metadata and entry point
- `tests/`: automated model and CLI tests

## Architecture and licensing note

The backend daemon is `wl-gammarelay-rs`, which is GPL-3.0-only. It is a
separate process that communicates with this plugin over DBus; this repository
does not link to or derive code from it.
